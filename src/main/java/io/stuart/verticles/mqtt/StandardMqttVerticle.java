...
public abstract class StandardMqttVerticle extends AbstractMqttVerticle {

    @Override
    public void start() throws Exception {
        super.start();

        Logger.log().info("Stuart's standard mqtt verticle start...");
        ...
        // Logger.log().info("Stuart's standard mqtt verticle prepared");
        server.endpointHandler(this::handleEndpoint).listen(ar -> {
            if (ar.succeeded()) {
                Logger.log().info("Stuart's standard mqtt verticle start succeeded, the verticle listen at port {}.",
                        port);
            } else {
                Logger.log().error("Stuart's standard mqtt verticle start failed, excpetion: {}.",
                        ar.cause().getMessage());
            }
        });
    }
    ...
    public void handlePublish(MqttEndpoint endpoint, MqttPublishMessage message) {
        ...
        vertx.executeBlocking(future -> {
            ...
            // topic is not blank and should not contain '+' and '#'
